...
ROSThread::ros_initialize(ros::NodeHandle &n)
{
  ...
  imu_pub_ = nh_.advertise<sensor_msgs::Imu>("/imu_raw", 1000);
  // ouster_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/os1_points", 1000); // giseop
  ouster_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/points_raw", 1000); // giseop
...
ROSThread::Ready()
{
  ...
  if(imu_active_)
  {
    ...
    while(1)
    {
      ...
      int length = fscanf(fp,"%ld,%lf,%lf,%lf,%lf,%lf,%lf\n", \
                          &stamp,&g_x,&g_y,&g_z,&a_x,&a_y,&a_z);
...
ROSThread::SaveOuster(rosbag::Bag &bag)
{
  ...
  for(auto && data_filending : ouster_file_list_){
    ...
    bag.write("/points_raw", publish_cloud.header.stamp, publish_cloud);
    cout<<"Written: "<<++count<<"/"<<ouster_file_list_.size() <<" /points_raw"<<endl;
...
void
ROSThread::SaveImu(rosbag::Bag &bag){
  FILE *fp;
  int64_t stamp;
  fp = fopen((data_folder_path_+"/sensor_data/xsens_imu.csv").c_str(),"r");
    double q_x,q_y,q_z,q_w,x,y,z,g_x,g_y,g_z,a_x,a_y,a_z,m_x,m_y,m_z;
    ...
    while(1)
    {
      ...
      bag.write("/imu_raw", imu_data.header.stamp, imu_data);
    }
    cout << "IMU data are saved" << endl;
    fclose(fp);
}
